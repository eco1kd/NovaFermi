...
static bool nv_fermi_dcb_header_plausible(const u8 *p) {
  ...
  if (entry_count < 1 || entry_count > 32)
    return false;
  if (entry_len != 1 && entry_len != 2 && entry_len != 4 && entry_len != 8 &&
      entry_len != 10)
    return false;
  return true;
}

static void nv_fermi_scan_dcb_candidates(struct nv_fermi_priv *priv,
  ...
  if ((size_t)bit_i->offset + dump_len <= priv->vbios_len) {
    ...
    print_hex_dump(KERN_INFO, DRV_NAME ": bit_i ", DUMP_PREFIX_OFFSET, 16, 1,
                   priv->vbios + bit_i->offset, dump_len, false);
  }
  ...
  for (i = 0; i + 2 <= dump_len; i += 2) {
    ...
    pr_info(DRV_NAME ": candidate @ bit_i+0x%02zx -> 0x%04x: version=0x%02x "
                     "header_len=%u entry_count=%u entry_len=%u %s\n",
  ...
  if (priv->vbios_len > 0x38) {
    ...
    if (legacy && (size_t)legacy + 4 <= priv->vbios_len) {
      ...
      pr_info(DRV_NAME ": legacy candidate @ 0x36 -> 0x%04x: version=0x%02x "
                       "header_len=%u entry_count=%u entry_len=%u %s\n",
...
int nv_fermi_parse_dcb(struct nv_fermi_priv *priv, struct nv_dcb_header *dcb) {
  ...
  if (nv_fermi_bit_find(priv, 'i', &bit_i) == 0) {
    ...
    if (bit_i.length >= 2 && (size_t)bit_i.offset + 2 <= priv->vbios_len) {
      bit_i_off =
          priv->vbios[bit_i.offset] | (priv->vbios[bit_i.offset + 1] << 8);
  ...
  pr_info(
      DRV_NAME
      ": DCB candidate from BIT 'i': 0x%04x (%s), legacy 0x36: 0x%04x (%s)\n",
      bit_i_off, bit_i_ok ? "plausible" : "not plausible", legacy_off,
      legacy_ok ? "plausible" : "not plausible");
  ...
  if (bit_i_ok) {
    ...
    pr_info(
        DRV_NAME
        ": BIT 'i' pointer not plausible, using legacy offset 0x36 instead\n");
  }
  ...
  if (dcb->header_len >= 8) {
    ...
    if ((size_t)dcb->i2c_table_off + 4 <= priv->vbios_len) {
      const u8 *ip = priv->vbios + dcb->i2c_table_off;
      u8 i_header_len = ip[1];
      u8 i_entry_count = ip[2];
      u8 i_entry_len = ip[3];
      size_t i_full_len =
          (size_t)i_header_len + (size_t)i_entry_count * i_entry_len;

      if ((size_t)dcb->i2c_table_off + i_full_len <= priv->vbios_len) {
        pr_info(DRV_NAME ": I2C table raw dump (%zu bytes at 0x%04x, "
                         "entry_count=%u entry_len=%u):\n",
                i_full_len, dcb->i2c_table_off, i_entry_count, i_entry_len);
        print_hex_dump(KERN_INFO, DRV_NAME ": i2c_tbl ", DUMP_PREFIX_OFFSET, 16,
                       1, ip, i_full_len, false);
      } else {
        pr_warn(DRV_NAME ": i2c table claims %zu bytes but that runs past the "
                         "VBIOS end, skipping dump\n",
                i_full_len);
      }
    } else {
  ...
  if (dcb->header_len >= 10) {
    u32 sig = p[6] | (p[7] << 8) | (p[8] << 16) | ((u32)p[9] << 24);

    if (sig == NV_DCB_SIGNATURE)
      pr_info(DRV_NAME ": DCB signature at +0x06 verified: 0x%08x\n", sig);
    else
      pr_warn(
          DRV_NAME
          ": DCB signature at +0x06 mismatch: got 0x%08x, expected 0x%08x\n",
          sig, NV_DCB_SIGNATURE);
  }
  ...
  if (dcb->header_len >= 12) {
    ...
    pr_info(DRV_NAME
            ": DCB gpio_table_off=0x%04x (confirmed by NVIDIA DCB 4.0 spec)\n",
            dcb->gpio_table_off);
    ...
    pr_warn(DRV_NAME
            ": DCB header_len=%u too small to contain gpio_table_off\n",
            dcb->header_len);
  }

  if (dcb->header_len >= 20) {
    dcb->i2c_devices_table_off = p[18] | (p[19] << 8);

    pr_info(DRV_NAME ": DCB i2c_devices_table_off=0x%04x (NVIDIA DCB 4.0 spec, "
                     "separate from CCB)\n",
            dcb->i2c_devices_table_off);

    if ((size_t)dcb->i2c_devices_table_off + 4 <= priv->vbios_len) {
      const u8 *dp = priv->vbios + dcb->i2c_devices_table_off;
      u8 d_header_len = dp[1];
      u8 d_entry_count = dp[2];
      u8 d_entry_len = dp[3];
      size_t d_full_len =
          (size_t)d_header_len + (size_t)d_entry_count * d_entry_len;

      if ((size_t)dcb->i2c_devices_table_off + d_full_len <= priv->vbios_len) {
        pr_info(DRV_NAME ": I2C Devices table raw dump (%zu bytes at 0x%04x, "
                         "entry_count=%u entry_len=%u):\n",
                d_full_len, dcb->i2c_devices_table_off, d_entry_count,
                d_entry_len);
        print_hex_dump(KERN_INFO, DRV_NAME ": i2c_dev_tbl ", DUMP_PREFIX_OFFSET,
                       16, 1, dp, d_full_len, false);
      }
    }
  }
